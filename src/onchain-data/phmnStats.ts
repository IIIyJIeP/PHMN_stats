import { StargateClient, QueryClient, createProtobufRpcClient } from "@cosmjs/stargate";
...
export async function getPhmnStats(): Promise<PhmnStats> {
    const [
        phmnTokenInfo, dasContractInfo,
        // phmnPoolsInfoOsmosis, weirdPoolsInfoOsmosis, phmn_price_osmo, phmn_price_ntrn
    ] = await Promise.all([
        getPhmnTokenInfo(),
        getDasContractInfo(),
    ...
    phmnStatsPoint.fields.push(
        ...
        {
            name: 'max_supply',
            value: phmnTokenInfo.maxSupply / 1e6
        },
        {
            name: 'current_supply_ratio',
            value: +(phmnTokenInfo.currentSupply * 100 / phmnTokenInfo.maxSupply).toFixed(2)
        },
        {
            name: 'phmn_das_treasury',
            value: phmnTokenInfo.dasTreasury / 1e6
        },
        {
            name: 'unbonding_period',
            value: (phmnTokenInfo.dasContractBalance - dasContractInfo.totalHolded) / 1e6
        },
        {
            name: 'total_staked_ratio',
            value: dasContractInfo.totalHolded * 100 / phmnTokenInfo.currentSupply
        },
    )
    ...
    for (let treasury of phmnTokenInfo.subDaoPhmnBalances) {
    ...
    const totalSubDaoTreasurysPhmnAmount = phmnTokenInfo.subDaoPhmnBalances.reduce((sum, subdao) => sum += subdao.phmnAmount / 1e6, 0)
...
async function getPhmnTokenInfo() {
    const phmnTokenInfo = {
    ...
    const owners = await getAllDenomOwners(cosmosRpcEndpoint, denomPHMNcosmos)
            ...
            if (amount > 0) {
                if (owner.address === DAS_CONTRACT_ADDRESS) {
                    ...
                    const subDao = subdaoTreasuryAddresses.find((subdao) =>
                        subdao.treasuryAddress === owner.address
                    )
                    if (subDao) {
                        phmnTokenInfo.subDaoPhmnBalances.push({
                            subdaoTag: subDao.subdaoTag,
                            phmnAmount: amount
                        })
                    } else {
                        phmnTokenInfo.phmnBalances.push({ address: owner.address, amount })
                    }
                }
            }
    ...
    phmnTokenInfo.currentSupply = Number((await getDenomSupply(cosmosRpcEndpoint, denomPHMNcosmos))?.amount || '0')
    phmnTokenInfo.maxSupply = phmnTokenInfo.currentSupply

    return phmnTokenInfo
}
...
async function getAllDenomOwners(rpc: string, denom: string): Promise<DenomOwner[]> {
    ...
    const owners: DenomOwner[] = [];
    let nextKey: Uint8Array = new Uint8Array();

    do {
        const res = await bank.DenomOwners({
            denom,
            pagination: {
                key: nextKey,
                offset: 0n,
...
async function getDenomSupply(rpc: string, denom: string): Promise<Coin | undefined> {
  const cometClient = await connectComet(rpc);
  const bank = new QueryClientImpl(createProtobufRpcClient(new QueryClient(cometClient)));
  const res = await bank.SupplyOf({ denom });
  cometClient.disconnect();
  return res.amount;
}